    ...
    return (
        <ul className="list-group">
            {props.list.length ? props.list.sort((a, b) => Date.parse(a.dueDate) - Date.parse(b.dueDate)).map((task: Task): ReactNode => {
                return <li key={task._id} className={`list-group-item ${task.completed ? "list-group-item-success": Date.parse(task.dueDate) < Date.now() ? "list-group-item-danger" : task.priority ? "list-group-item-warning" : "list-group-item-info"}`}>
                    <div>
                    ...
                    <button data-toggle="collapse" data-target={`#${task._id}notes`}>Details</button>
